 ...
 * （純文字類／.docx／.doc／.pdf／.pptx／.xlsx／.tex／.ipynb／.py／.json／.csv，見 ai_bridge._SEED_EXTS），一次呼叫產出一批任務便利貼草稿。使用者逐則勾選／
        ...
        {phase === 'pick' && (
          <>
            <p className="dim">
              ...
              （<code>.md</code>／<code>.txt</code>／<code>.rst</code>／<code>.csv</code>／
              <code>.py</code>／<code>.json</code>／<code>.tex</code>／<code>.ipynb</code>／
              <code>.docx</code>／<code>.doc</code>／<code>.pdf</code>／<code>.pptx</code>／
              <code>.ppt</code>／<code>.xlsx</code>）產出任務便利貼草稿。
            </p>
