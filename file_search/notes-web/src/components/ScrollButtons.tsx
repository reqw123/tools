...
/**
 * 右下角浮動的「捲到最上面 / 最下面」——內容比一個畫面還高才出現（牆很長時
 * 才有意義）。雙牆（notes-web / files-web）各一份，是既有的不共用前端慣例。
 */
export function ScrollButtons() {
  const [scrollable, setScrollable] = useState(false)

  useEffect(() => {
    // 牆的高度由 Wall.tsx 量完卡片後用 JS 寫死，不會觸發 resize/scroll——用
    // ResizeObserver 盯 body 高度變化來重新判斷要不要顯示。
    const check = () =>
