 ...
 * `public-wall-data/`（區網＋公網共用模式下的資料夾，見 `store.ts` 的
 * `notesFilePath`）完全在 `.gitignore` 外、沒有任何備份機制，資料損毀時
 * 求助無門。把整個資料夾（便利貼 JSON、圖片、標籤色、`.share/`）打包成
 * 一個 `.zip`，讓 `/host` 的人直接下載。搬自
 * `files-web/server/backup.ts`，邏輯完全相同。
 *
 ...
 *  （便利貼 JSON＋少量插圖），先整包收進記憶體比較單純，不必為了這種規模
 *  另外做串流下載的複雜度。資料夾不存在／完全是空的都回傳一個空 zip
