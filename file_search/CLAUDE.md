...
| Web 索引牆／便利貼牆 訪客紀錄自帶牆別標記 | **2026-09 修的真實 bug**（Node-RED「多人牆訪客通知」分頁）：一開始靠「Node-RED 打了哪個 port」判斷一筆造訪是哪面牆——分別打 notes-web（8792）／files-web（8793）的 `/api/host/visits`，用兩個「change」節點各自標 `msg.wall='notes'\|'files'`。使用者反映「discord 傳送是相反的」（切一次牆卻兩面各發一則、牆別看起來還會反過來）。**根因**：多人牆閘道模式下兩邊的 `.share/visits.json` 其實是**同一個實體檔案**（`serve.mjs` 把兩邊資料夾都指到同一個 `public-share-data/`，見「Web 多人牆閘道」那列的「共用登入／身分」段落——當初就已經點出 `.share/people.json` 是同一份，這次踩到的是同一類坑但換成 `visits.json`），打 8792 或 8793 讀到的其實是同一份合併清單，「打哪個 port 就標哪面牆」從來就不是可靠依據——同一筆造訪會被兩個分支同時當「新的」，各發一次，兩則的牆別純粹看哪個分支先跑到，跟這筆造訪實際發生在哪面牆完全無關。**正確修法在後端**：`notes-web/server/visits.ts`／`files-web/server/visits.ts` 的 `VisitEntry` 加一個 `wall: 'notes'\|'files'` 欄位，`recordVisit()` **寫死**（不是猜、不是看環境變數，各自的程式碼寫死自己是哪面牆），`/api/host/visits` 回傳的每一筆自此自己講清楚是哪面牆，不管兩邊實體檔案是不是同一份都不受影響。Node-RED 那邊因此**改回只打一個 port**（8792，兩份資料本來就一樣，打哪個都一樣）拿完整清單，「偵測新造訪紀錄」function 改讀每筆自帶的 `v.wall`（舊資料沒有這欄位的，沿用先前預設值 `'notes'`，不會把歷史資料判錯）。**教訓**：閘道模式下有好幾份資料被刻意共用同一個實體檔案（`people.json`／`visits.json`），任何要「分辨這筆資料屬於哪一面牆」的需求都必須在**寫入當下**由各自後端寫死標記，不能事後靠「這筆資料是從哪個 API/port 讀到的」反推——這類共用檔案兩邊 API 讀到的內容本來就一模一樣，port 不帶任何區分資訊。 |
...
| Web 索引牆 匯入資料夾進度條 | **2026-09 加**，使用者反映「離線牆匯入資料夾檔案眾多時處理很久，要有進度條避免誤會當機」。**慢的是「掃描」不是「匯入」**——`scanFolder()`（`server/store.ts`）遞迴走檔最多 20 萬個項目，原本是**完全同步**：整個 Node 事件迴圈被占住，不只前端只能轉圈圈，連 SSE／別的請求都卡死，所以光在前端加動畫沒用，一定要先讓掃描能讓出事件迴圈才有進度可回報（寫入 `.md` 的 `appendEntries()` 是一次讀寫，很快，只給它一條不定長進度條）。**改法**：`scanFolder` 改 `async`，每連續跑 `SCAN_SLICE_MS`（30ms）就 `onProgress()` ＋ `await setImmediate` 讓出一次，順便檢查 `isCancelled()`；`server/scan-jobs.ts` 是記憶體內的工作表（`startScanJob`／`getScanJob`／`cancelScanJob`，完成後留 10 分鐘），路由 `POST /scan/jobs`（202 ＋ id）／`GET /scan/jobs/:id`（輪詢）／`DELETE /scan/jobs/:id`（取消）。舊的一次等到底 `POST /scan` 留著（README 有記載）但前端不再用。**`share.ts` 的危險端點擋板要用 `path.startsWith('/api/scan/')` 一併擋**，不然遠端可以繞過 `/api/scan` 那條，用新路由掃主機硬碟。前端：`useScan()`（`hooks/useIndexes.ts`）對外介面刻意維持跟 `useMutation` 一樣（`mutate`／`reset`／`isPending`／`data`／`error`），另加 `progress`／`cancel`，底層是 250ms 輪詢；`components/ProgressBar.tsx` 有 `value` 畫比例、沒有就畫不定長條（遞迴掃描事先不可能知道總數，硬湊百分比只會騙人，所以靠即時計數＋已花秒數＋目前資料夾讓人確定「真的還在動」；只有「不含子資料夾」才有 `total`）。掃描中鎖住類型按鈕／「包含子資料夾」（改條件本來就會作廢結果，不鎖的話手滑等於默默取消掃描）。**踩過的坑（UI 測試抓到）**：`cancel` 一開始只中止輪詢迴圈、沒把狀態設回 idle，取消後進度條和「掃描中…」永遠卡著——對外的 `cancel` 必須等於 `reset`（中止＋回 idle），只有「換一輪／卸載」才用不 setState 的內部 `abort`。**已驗證**：隔離 server 上 3 萬檔／C:\Windows（撞 20 萬上限，約 7.7 秒）——進度單調遞增、掃描期間輪詢延遲 ≤ 61ms、取消、不存在／相對路徑 400、未知 id 404、舊 `/scan` 仍可用；Playwright 開瀏覽器實測掃描中計數即時跳動、完成後進度條消失、取消後回到可重新掃描，console 無錯誤。**注意**：build 後要重啟 server（`@fastify/static` 啟動時就登記了檔名，重 build 出新 hash 的檔案它不認，會回 index.html 造成 MIME 錯誤）。 |
| Web 索引牆 上傳資料夾進度條 | **2026-09 加**，接在「匯入資料夾進度條」（上一列）之後，使用者要求「上傳資料夾」按鈕（`UploadFolderDialog.tsx`）也一併處理。跟匯入不同：慢的是**上傳位元組**，不是掃描，所以有真正的百分比。`fetch` 拿不到上傳進度，只有 `XMLHttpRequest.upload.onprogress` 有——`api.ts` 新增 `postFormWithProgress()`（`uploadFolder()` 改走它），跟 `req()` 共用新抽出的 `errorFromResponse()`（401 needAuth → `AuthError`、其餘取 `error` 欄位），避免兩邊各寫一份錯誤對照。回報的 total 是整個請求本體（含 multipart 邊界）大小，比檔案總和略大一點；「約第 N / M 個檔案」是用「已送比例 × 檔案總位元組」對各檔累計大小換算的估計值，UI 一律標「約」。位元組全部送完後 server 還要收尾（寫索引、回應），這段沒有位元組可報，所以 `allSent` 時進度條改畫不定長＋「檔案已傳完，伺服器正在寫入索引…」，不讓 100% 卡著像當機。**刻意沒有取消鈕**：`upload-batch`（`upload-routes.ts`）是邊收邊寫檔，`for await` 被中斷會落到 catch「用已經收到的繼續」——客戶端一中止，已收到的那一部分會被當成一整批加進索引（部分匯入），比「上傳中不能關」更令人意外，所以維持原本上傳中鎖住關閉／Escape。上傳中鎖住類型按鈕（請求已送出，改篩選只會讓畫面數字跟實際送出的不一致）。**選資料夾階段沒辦法做進度**：`<input webkitdirectory>` 的 `change` 要等瀏覽器把整個資料夾讀完才觸發，之前沒有任何事件可掛，所以只在選擇畫面加一行靜態提示（「檔案很多時瀏覽器讀取需要一點時間，不是當機」）。**已驗證**：隔離 server＋Playwright，用 CDP 把上傳限速到 300KB/s（本機回環太快看不到進度條），200 個檔案／5.9MB 的資料夾（含一個 `node_modules` 垃圾夾）：百分比、已傳位元組、約第 N 個檔案隨時間平順增加、`node_modules` 被排除、完成後對話框自動關閉、索引集正好多 200 筆，console 無錯誤。 |
| Web 索引牆 預覽數量上限＋釋放全部 | **2026-09 加**，使用者問「匯入很多影片路徑、每個都預覽後暫停，會同時持續占資源嗎」，實測後要求「一次只保留 10 個預覽（開新的自動收掉最舊的）＋一個釋放所有資源的按鈕」。**實測結論（隔離 server＋無頭 Chromium＋ffmpeg 產生的 720p VP8 webm）**：暫停後 CPU 為 0、網路連線回到基準、伺服器檔案控制代碼回到基準、其他 API 延遲不受影響——**唯一的持續成本是瀏覽器記憶體**，每個還掛著的 `<video>` 約 +25–35MB，而且**光是 `preload="metadata"` 載入影片資訊（還沒按播放）就會占**。沒測到：H.264 硬體解碼（無頭 Chromium 不支援），那種情況占的是顯示卡記憶體、同時可用的解碼器數量有上限，數字不同。**做法**：預覽開關從 `EntryRow` 的本地 state 搬到 `EntryList`（`previewIds`，由舊到新，`MAX_PREVIEWS = 10`），`EntryRow` 改吃 `preview`／`onTogglePreview` props；`FocusedEntry`（懸浮小視窗，一次只有一列）自己放一個本地 state，不套用上限。**id＝列 key＋序號／分類／說明**——內容一變 id 就對不上、預覽自動收起，等於保留了以前 `EntryRow` render 階段偵測 `rowSig` 變了就 `setPreview(false)` 的行為，不用另外偵測。**「開著幾個」只算真的還掛在畫面上的**（`liveIds`：那一列在目前篩選結果裡、所在分組沒收合、列是展開的）——被篩掉／分組收合／列收合的列 `<video>` 早就卸載，不占資源，不該吃掉額度；每次寫入先剔掉這些舊 id 再套用上限，所以已知不變式是「開著的預覽 ≤ 10」。「釋放全部預覽資源」是 `.preview-release`——**一顆 2.4rem 的半透明圓鈕（圖示＋右上角數字徽章，說明只放 title／aria-label）**，有預覽開著才出現，固定在右下角、緊貼捲動鈕（`.scroll-fab`）左邊，同一套 FAB 樣式（`opacity: .55`，hover 才變清楚）。**教訓**：第一版是左下角一條帶文字的浮動小條（「已開啟 N / 10 個預覽（超過會自動收掉最舊的）」＋按鈕），使用者反映「太佔畫面、會擋到內容」——浮動在內容上的東西要盡量小、無文字、半透明，說明放 tooltip，不要為了「說清楚」直接把字印在畫面上。 `FilePreview.tsx` 的 `<video>`／`<audio>` 加了 ref callback 清理（`releaseMedia`：卸載時 `pause()`＋`removeAttribute('src')`＋`load()`，React 19 的 ref callback 可回傳清理函式），讓瀏覽器立刻中斷請求、放掉解碼器，不用等垃圾回收。**已驗證**（14 部影片、Playwright）：依序開 14 個，任何時候 DOM 裡 `<video>` 都 ≤ 10，開到 #14 時剩第 5–14 列（第 1–4 列被收掉，按鈕變回「預覽內容」）；「釋放全部」後 `<video>` 歸零、小條消失、14 列仍展開；收合一列帶預覽的列，計數立刻 −1；console 無錯誤。**記憶體的誠實數字**：10 個預覽播放後暫停約 880–900MB（基準約 320–340MB），按釋放後降到約 655MB 就**不再下降**（等到 90 秒也一樣）——Chromium 處理程序不會把記憶體還給系統，所以「釋放」拿不回全部；但連做 4 輪「開 10 個→釋放」，釋放後穩定在 652–660MB、開著時穩定在 875–900MB，**不會一輪一輪疊高**，這是上限＋釋放真正帶來的保證（有界，不是回到零）。 |
| Web 索引牆 懸浮視窗（`FocusedEntry`）拖曳把手捲動後失效 | **2026-09 修的真實 bug**（使用者回報「被懸浮成獨立視窗的索引項目，內容捲到下方就無法靠滑鼠移到邊界拖動視窗，要捲回最上方才有效」）。**根因**：`.focused-entry` 自己是 `overflow: auto` 的捲動容器，四條透明拖曳把手（`.focused-drag-handle`／`.focused-drag-edge`，Electron 的 `-webkit-app-region: drag`）和右上角「↩ 收回」鈕都是它裡面的 `position: absolute` 子元素，**絕對定位的子元素會跟著捲動容器的內容一起捲走**——實測（視窗 520×420、內容多 139px）捲到底後上緣把手 y = −139（在畫面外）、左右邊緣只剩上半段（到 y=265，下面 155px 沒有）、下緣跑到視窗中間，**連收回鈕也被捲出畫面**。只有 files-web 有這問題：notes-web 的 `.focused-note` 高度固定、不捲動。**修法**：外層 `.focused-entry` 改成 `overflow: hidden`（不捲動），新增內層 `.focused-scroll`（`position: absolute; inset: 0; padding: .35rem; overflow: auto`）負責捲動，把手跟收回鈕留在外層固定不動。**副作用要顧**：以前把手在捲動容器裡，`right: 0` 自動落在捲軸左邊（絕對定位的包含區塊不含捲軸）；現在把手在外層，會蓋到內層捲軸讓捲軸拖不動——本專案捲軸是自訂的 14px（`::-webkit-scrollbar`），`FocusedEntry.tsx` 用 `useLayoutEffect`＋`ResizeObserver`（觀察捲動區與其內容，因為捲軸出現／消失取決於內容高度）量出 `offsetWidth − clientWidth`／`offsetHeight − clientHeight`，寫成外層的 CSS 變數 `--sb-x`／`--sb-y`，右／下緣的把手與收回鈕用它讓開。**已驗證**：Playwright（要 `ignoreDefaultArgs: ['--hide-scrollbars']` 才有真實捲軸，無頭模式預設隱藏捲軸，量出來 `sb: 0` 會誤以為沒問題）——先在舊 build 重現（把手跑到畫面外），修完後捲到底四條把手與收回鈕位置跟 scrollTop=0 時完全一樣，右緣把手在 x=492–506、捲軸在 506–520 不重疊，`elementFromPoint` 在左下／下緣中間／上緣中間都命中對的把手。**後續**：Playwright 測不到 Electron 專有的行為，這條的「實際拖曳」後來改用真的 Electron＋真滑鼠驗證，並發現原本用 `-webkit-app-region` 的做法本身就有更大的問題，整個換掉了——見下一列。 |
| 懸浮視窗（索引項目／便利貼）拖曳把手：游標不變、拖不動 | **2026-09 修**（使用者要求「確定滑鼠放在可移動視窗位置的地方時，游標圖案會改變並且真的能移動」）。**先實測基準**（用 wallpaper-app 自己的 Electron 開一個跟 `createPinnedWindow()` 選項一樣的視窗，用真的作業系統滑鼠 `SetCursorPos`／`mouse_event` 移上去、`GetCursorInfo` 讀目前游標圖案、實際按住拖動量視窗位置）：游標停在拖曳把手上讀到的是**一般箭頭**（靠邊緣時是系統的縮放圖案），從來不是「移動」；實際拖動也不穩（捲動前第一次拖動完全沒動、從左緣拖只動了 20px / 預期 140）。**根因**：`-webkit-app-region: drag` 是交給作業系統原生處理的，網頁完全收不到滑鼠事件——`:hover` 高亮、`cursor: move` 這兩條原本寫好的 CSS 根本沒生效，而且靠近視窗邊緣時會跟原生縮放邊框搶事件。**做法**：把手改成一般網頁元素（`cursor: move`＋hover 高亮自然生效），`lib/windowDrag.ts`（notes-web／files-web 各一份）的 `windowDragHandlers` 收 pointer 事件：按下 → `setPointerCapture` ＋ `window.desktopWall.dragWindowStart()`，放開／取消／失去捕獲／視窗失焦／`pointermove` 的 `buttons===0` → `dragWindowEnd()`（`end()` 冪等）。主行程新模組 `wallpaper-app/window-drag.js`（`main.js` 呼叫 `registerWindowDrag()`，`wall-preload.js` 開 `dragWindowStart/End` 兩個方法）：每 8ms 讀 `screen.getCursorScreenPoint()`，把視窗搬到「起始位置＋游標位移」——用游標**絕對座標**不累加 mousemove 位移（視窗自己在動，累加會漂移），每次都帶原本的 width/height 呼叫 `setBounds`（跨不同縮放比例的螢幕時 Windows 會悄悄改視窗尺寸的已知問題）。角落刻意不放把手（留給原生縮放）；**最外緣約 1px 仍是系統原生縮放邊框**（實測 SIZEWE／角落 SIZENWSE），不衝突。**踩到的退步（實測抓到）**：便利貼視窗裡的對話框（`NoteDialog`）遮罩 `.scrim`（z-index 100）蓋在把手（原本 6）上面，對話框開著就完全拖不動——以前 app-region 由作業系統判定、不受網頁元素覆蓋影響，改成一般元素後才出現。把手抬到 z-index 300（蓋過 `.scrim` 100／`.zoom-scrim` 200）、收回鈕 301，跟舊行為（拖曳區蓋過網頁內容）一致。**主行程的保險**：除了收網頁送來的 `wall-drag-end`，還在拖曳期間直接監聽該 webContents 的 `before-mouse-event`，`mouseUp` 或 `mouseDown`（再按下＝前一次放開一定漏掉了）就停；另有 60 秒硬上限。**已驗證**（真滑鼠）：三種視窗——索引項目（捲動前／捲到底）、便利貼、便利貼＋對話框開著——上／左／下（及右）把手第一次悬停就是 SIZEALL（移動）游標、最外緣是系統縮放、收回鈕是手指；實際從上／左／下緣拖動，視窗精確移動 (140, 90)、尺寸不變；放開後視窗停住（游標再移開也不動）；連續多次拖動、大步跳遠、快速甩動都正常結束（`b2b` 6/6）。**已知未解的現象（不要當成已修好）**：某支特定腳本（拖動 → 放開 → 隔約 0.6 秒 `reset-pos` → 立刻「先按下、100ms 後一大步跳 600px、300ms 後放開」）會**決定性地**重現「放開事件沒送到」——連 Electron 主行程的 `before-mouse-event` 都沒看到 `mouseUp`（所以網頁與主行程的保險都無從觸發，視窗會一直跟著游標直到下一次點擊）；同樣的動作換個前置狀態（單獨跑、前面加長停頓、`b2b` 的 S3/S5）就正常，二十多次其他組合全部正常，沒找到根因，推測是合成輸入（`mouse_event`）與視窗剛被程式搬移的時序在作業系統輸入層的問題，真人滑鼠是連續事件流未必會遇到。**請在真實桌面版上實際拖幾次確認**。**測試方法**（想再驗證時）：`ETEST_*` 環境變數 harness＝用 wallpaper-app 的 `node_modules/electron/dist/electron.exe` 跑一支自製 main.js（`app.setPath('userData', <暫存資料夾>)` 隔離，不碰使用者真正在跑的 app；載入 `wall-preload.js` 與真的 `window-drag.js`）＋ PowerShell `Add-Type` P/Invoke `user32`（`SetProcessDPIAware`、`SetCursorPos`、`GetCursorInfo` 比對 `LoadCursor(IDC_SIZEALL=32646)`、`mouse_event`、`WindowFromPoint`）。要點：(1) 滑鼠按下前先用 `WindowFromPoint`＋`GetWindowThreadProcessId` 確認游標底下真的是測試視窗，避免點到使用者的其他視窗；(2) 判斷 pid 時 `[uint32]` 對 JSON 讀出的 `[int64]` 直接 `-eq` 會出錯，要兩邊都轉 `[int64]`；(3) 剛開的視窗第一次悬停，`SetCursorPos` 一次跳過去只產生一個 mouse move，游標圖案要等下一個事件才更新，要像真人一樣連續 `SetCursorPos` 一步步移入才準；(4) `webContents` 的 `cursor-changed` 事件反映的是網頁設的游標，不是作業系統實際顯示的，要看 `GetCursorInfo`；(5) 這個工具環境會把 PowerShell 命令裡的 `\d+` 或 `Remove-Item Env:` 誤判成要刪除的路徑而整個擋下，得拆開命令或改寫。 |
| 桌面牆產品展示腳本（`wallpaper-app/demo/`） | **2026-09 加**，使用者要求「用剛剛的開發過程展示 AI 可以操控電腦」。`run.ps1` 用**真實滑鼠鍵盤**（`user32` 的 `SetCursorPos`／`mouse_event`／`SendInput`）把「開離線牆 → 新增便利貼 → 拖出去懸浮 → `Shift+C` 切索引牆 → 新增索引集 → 匯入資料夾 → 預覽影片 → 懸浮 → 拖邊界擴大 → 播放 N 秒停止 → 按 ✕ 結束」走一遍，約 1 分鐘，參數化（`-ImportDir`／`-VideoFile`／`-PlaySeconds`／`-IndexName`／`-NoCleanup`），跑完預設呼叫 `cleanup.py` 清掉這次新增的資料。**兩個設計決定**：(1) 元素位置靠 Electron 的 `--remote-debugging-port=9333`（`npm start -- --remote-debugging-port=9333`，只綁本機）用 CDP 的 `Runtime.evaluate` 查 DOM 位置（`cdp.ps1`），只「讀位置」，點擊仍是真滑鼠；這是為了不用每一步截圖給模型看（原本逐步截圖版 6.6 分鐘、30 步，改腳本後 51～56 秒）；(2) 清理用 **Python** 不是 PowerShell——`.sticky_notes.json`／`settings.json` 要原樣寫回，PowerShell 5.1 的 `ConvertTo-Json` 會整份重新排版；先驗證「讀進再寫出」與原檔位元組一致才動手，只動精確比對得到的項目，索引集只在所有資料列都在 `--import-dir` 底下時才刪，桌面版還開著時拒絕執行。**這是「重播」不是「AI 自主操作」**——腳本知道所有按鈕名稱、走網頁側門讀 DOM 而不是看畫面、遇到意外只會失敗（使用者質疑過這點，見 `demo/README.md` 最後一節，對外展示要把「AI 先自己摸索」與「再固化成一鍵重播」分開講）。**踩過的坑**：`.ps1` 含中文一定要存 UTF-8 with BOM（PowerShell 5.1 讀無 BOM 檔當 ANSI，整支解析失敗）；`Invoke-RestMethod` 把 JSON 陣列當一個物件，要 `foreach` 展開；匯入資料夾必須在 下載／文件／家目錄 底下（檔案瀏覽器的捷徑，腳本從最近的捷徑一層層點進去）；拖出便利貼後用 CDP 靜默把 `localStorage['note-collection']` 改回 `thesis`（非畫面操作）避免下次啟動停在「生活」分頁。**已驗證**（貓咪自拍影片資料夾 35 檔、`VID20260904143024.mp4` 是 HEVC 1920×1080 89.7 秒）：全程 55.9 秒，影片實際播 4.30 秒後暫停（`readyState=4`、無錯誤碼、`canPlayType(hvc1)=probably`），視窗被拖大 1018×732 → 1358×953，清理後資料乾淨。**還原範圍（後續補強，使用者問「能自動清理與還原操作產物嗎」）**：除了資料檔案，桌面版還把**介面記憶存在網頁 localStorage**（便利貼牆 8787：`note-collection`＝生活/研究生分頁、`sticky-wall-default-tag`…；索引牆 8788：`index-wall-last`＝上次選的索引集、`index-wall-last-dir`＝檔案瀏覽器上次的位置…），展示會改到它們——一開始只還原了分頁，**查出索引牆的兩筆被改掉沒還原**（下次開啟會停在一份已刪除的索引集）。現在 `run.ps1` 在牆載入後先 `Get-Storage` 記下 8787，切到索引牆後再記 8788（此時還沒動過東西），便利貼拖出去後還原 8787、按 ✕ 前還原 8788 並 `Storage-Equal` 驗證（`cdp.ps1`）。**前後對照實測**（存基準 → 跑 → 再存 → 逐鍵比對）兩面牆完全一致。**中途失敗安全網**：主流程包在 `try/catch`，失敗時存整螢幕截圖＋對話框狀態（`out/shots/failure.png`）、依目前顯示哪面牆還原對應的記憶（讀回驗證＋等 1.5 秒讓瀏覽器寫入磁碟）、用 `dwQuit.quit()`（CDP 呼叫結束按鈕頁面，不靠滑鼠）關閉、跑清理、結束碼 1；`-KeepAppOnFailure` 可留著現場。已實測兩種失敗位置（便利貼牆／索引牆）都正確收尾。**打字讀回確認（`Type-Into`）**：實測遇過 SendInput **掉字**（標題少了最後一個「示」，存進去變成殘缺標題），所以每個輸入欄位打完都讀回 `.value` 比對，不一致全選重打最多 3 次。**清理不能只認完整標題**：殘缺標題的便利貼會被漏掉（真的漏過一張），`cleanup.py` 改成認「這次執行期間新增（`created_at` ≥ `--since`）且帶展示特徵（完整標題／標籤「展示」／內文開頭句）」，`--since` **必填**（不給的話範圍變成所有時間，可能誤刪使用者標籤剛好叫「展示」的舊便利貼）；超過 3 張符合就中止。**踩過的坑**：(1) 桌面版會把「上次停在哪面牆」寫進 `settings.json` 的 `wall`——只讀資料時用 `Shift+C` 切牆再關，下次會開在另一面、腳本找不到便利貼牆分頁而逾時，要切回原本那面再關；(2) 有一次失敗收尾後分頁記憶停在「生活」，**之後重複測試沒能重現**，原因沒查明（懷疑還原後太快關閉、localStorage 還沒寫到磁碟），所以加了讀回驗證與 1.5 秒等待，這個結論不要當成已根治。 |
