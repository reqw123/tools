...
class _NoTranscription:
    ...
    transcription_service 是 None 而炸掉。跟 index-wall-web 的 ai_bridge.py
    同一份。"""
...
def _ai() -> AIDescriptionService:
    ...
    # 「AI 生成便利貼」要從檔案內容擷取文字／圖片，這裡補上跟 index-wall-web
...
def cmd_generate_note(payload, notes_file):
    ...
    index-wall-web 的 `cmd_generate_note` 同一套邏輯（`_generate_one` 帶入
