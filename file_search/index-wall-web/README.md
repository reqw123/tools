...
跟便利貼版 [`sticky-wall-web`](../sticky-wall-web) 是**同層的兩個獨立分流**：
共用技術棧、各自一個資料夾、不共用程式碼。改**路徑**、前言 prose、
索引集（`.md` 檔本身）的建立／刪除、AI 全文搜尋等仍只在桌面版 `file_search.py`。
...
- **AI 全文搜尋、改路徑、前言 prose 編輯、索引集（`.md` 檔）的建立／刪除、
  匯出、全文快取、重複偵測、清除失效項目**：一律不做——那些是桌面版的職責。
