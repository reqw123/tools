...
AI）、批次分類（一顆按鈕兩個分頁：補上空白的／舊分類→新分類）、批次刪除（一律
只動 `.md` 表格列，不碰實體檔案——見 `docs/adr/0002`）。
索引集層級：匯入（=建立）／匯出（`docs/adr/0003`），以及新增空白 `.md`／
刪除整份／用系統文字編輯器開這份 `.md`（`docs/adr/0004`）。跟便利貼版
`notes-web` 是同層的兩個獨立分流，共用技術棧、不共用程式碼。「批次補說明」的
AI 模式跟便利貼牆一樣，靠 `server/ai_bridge.py` 子行程呼叫桌面版的
