...
export function BatchDescribeDialog({
  ...
  const runAi = async () => {
    setAiConfirm(false)
    const targets = rows.filter((r) => r.on)
    ...
    for (let k = 0; k < targets.length; k++) {
      ...
      try {
        ...
        if (res.skipped) skipped++
          ...
          ok++
          set(r.serial, { desc: res.suggestion })
        }
      } catch (e) {
        failed++
          ...
          {isLoading ? (
            ...
            <>
              ...
              <p className="sub mono">
                {query ? `符合 ${matched.length} / ${rows.length}` : `共 ${rows.length} 筆說明是空的`} · 已勾選 {checked.length}
                  ...
                  <div key={r.serial} className={`bd-card${r.on ? '' : ' off'}`}>
                    <label className="bd-card-head">
                      <input
                        ...
                        checked={r.on}
                        disabled={aiRunning}
                        onChange={(e) => set(r.serial, { on: e.target.checked })}
                      />
                    ...
                    <textarea
                      ...
                      onChange={(e) => set(r.serial, { desc: e.target.value })}
                    />
                  </div>
                ))}
