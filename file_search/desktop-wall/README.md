...
把 **便利貼牆**（`sticky-wall-web`）或 **檔案索引牆**（`index-wall-web`）貼成
...
好三個專案的依賴（desktop-wall + sticky-wall-web + index-wall-web）、build 出兩個
web `dist/`，並在**桌面建立捷徑**（圖示取自 `desktop-wall/assets/shortcut-icon.jpg`，
...
> `.bat` 在兩個 `dist/` 都存在時會**跳過** build。改了 `sticky-wall-web` /
> `index-wall-web` 的前端後，刪掉它們的 `dist/` 再跑 `.bat`，或直接
...
| 動作 | 預設鍵 |
|---|---|
| 切換 互動 / 背景 模式 | `Ctrl+Alt+W` |
| 顯示 / 隱藏 | `Ctrl+Alt+H` |
| 切換 便利貼牆 ⇆ 索引牆 | `Ctrl+Alt+S` |
| 開 / 關設定視窗（開關式：已開就關） | `Ctrl+Alt+O` |
...
- **設定檔位置** — `%APPDATA%\desktop-wall\settings.json`（含 `shortcuts` 欄位）
...
產出的 `desktop-wall.exe` **要放在 `file_search/` 底下**（跟 `sticky-wall-web` /
`index-wall-web` 同層），`servers.js` 才找得到它們（`path.join(__dirname, '..',
'sticky-wall-web')`）。web 專案的 `dist/` 不打進 exe——它們會自己更新。
