...
function currentUrl() {
  ...
  // 用這個初始化 croppedIds（見 sticky-wall-web 的 App.tsx）。視窗本身的
...
// ── 便利貼牆：拖框裁切 / 拖出去變懸浮視窗 ─────────────────────────────
// 兩個手勢的畫面/判斷邏輯都在 sticky-wall-web 那邊（見它的 App.tsx /
...
// ids 一併存進 store，重開 desktop-wall 時才知道要把哪幾則帶回裁切視窗
...
// （見 sticky-wall-web 的 main.tsx / FocusedNote.tsx）——不用另外做一套
...
// 索引集底下查得到（見 sticky-wall-web/index-wall-web 的 useIndex() 快取）。
...
ipcMain.handle('wall-pin-entry', (_e, entry, rect) => {
  // entry: { indexName, path }——見 index-wall-web 的 App.tsx onPin。
...
function runShortcut(action, meta) {
  try {
    ...
    try {
      ...
      console.warn(`[desktop-wall] 快捷鍵提示求值失敗（${action}）：`, err.message);
    }
...
function registerShortcuts() {
  ...
  for (const [action, meta] of Object.entries(SHORTCUT_META)) {
    ...
    if (bad) {
      ...
      console.warn(`[desktop-wall] 快捷鍵「${accel}」（${meta.label}）：${bad}`);
      continue;
    }
    const norm = normalizeAccel(accel);
    if (seen.has(norm)) {
      ...
      console.warn(`[desktop-wall] 快捷鍵「${accel}」重複（${seen.get(norm)} / ${action}）`);
      continue;
    }
    let ok = false;
    try {
      ...
      console.warn(`[desktop-wall] 快捷鍵「${accel}」註冊丟錯：`, err.message);
    }
    ...
    if (ok) {
      ...
      console.warn(`[desktop-wall] 快捷鍵「${accel}」（${meta.label}）註冊失敗`);
    }
  }
  refreshTray();
  pushSettingsState();
...
app.whenReady().then(async () => {
  ...
  try {
    ...
    console.error('[desktop-wall] web server 沒能就緒：', err.message);
  }
  createWall();
  createHintOverlay();
  createQuitButton();
...
// 視窗全關不退出——這是常駐工具，靠系統匣「結束」或 Ctrl+Alt 快捷鍵才收。
...
app.on('preload-error', (_e, p, err) => {
  console.error('[desktop-wall] preload 載入失敗：', p, '\n', err);
});
