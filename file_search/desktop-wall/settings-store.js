'use strict';
// desktop-wall 自己的設定檔——app.getPath('userData') 會依這個行程的 package.json
// name（"desktop-wall"）算路徑，跟 desktop-pet / control-center 各自獨立，不會混。
...
const DEFAULT_SHORTCUTS = Object.freeze({
  toggleMode: 'Control+Alt+W', // 切換 互動 / 背景
  toggleVisible: 'Control+Alt+H', // 顯示 / 隱藏
  switchWall: 'Control+Alt+S', // 切換牆
  openSettings: 'Control+Alt+O', // 開設定視窗
});
...
function read() {
  ...
  try {
    ...
    if (err.code !== 'ENOENT') {
      console.warn('[desktop-wall] settings.json 讀取失敗，已退回預設值：', err.message);
    }
  }
...
function write(patch) {
  ...
  try {
    ...
    console.error('[desktop-wall] settings.json 寫入失敗：', err.message);
  }
  return next;
}
