...
_HELP_SECTIONS = [
    ...
    ("🛠️ 索引管理", [
        ...
        (BTN_EDIT_BG, "🏷️ 批次改分類...",
         "勾選項目、統一改成同一個分類（留空＝未分類），一次套用到所有勾選的列；只改分類欄，路徑／說明都不動。"),
    ]),
    ...
    ("📌 便利貼（常駐左側面板，記常用指令／網站／備忘）", [
        ...
        (BTN_EDIT_BG, "🏷 批次改標籤",
         "跳出可搜尋、可勾選的清單，統一把勾選的便利貼改成同一個標籤（留空＝清空標籤）；"
         "只改標籤，標題／內容／到期日都不動。"),
        (BTN_DANGER_BG, "🗑️ 批次刪除",
         "跳出可搜尋、可勾選的清單批次刪除便利貼；預設全部不勾選，需再次確認才會真的刪除。"),
        (BTN_SECONDARY_BG, "♻ 垃圾桶",
         "刪除（單筆或批次）的便利貼會先放進這裡，不是直接消失；可以逐筆復原，或永久刪除、清空垃圾桶。"),
        (BTN_EDIT_BG, "📝 編輯便利貼檔案",
