...
class StickyNoteService:
    ...
    def build_ai_search_prompt(self, notes: list, query: str) -> str:
        ...
        lines = [
            ...
            "類型包括：①找出符合某個描述、需求或情境的便利貼、②某個分類/主題"
            "底下實際有哪些事項或內容（要把相關便利貼的內容重點統整寫進答案，"
            "不能只回「有」）、③數量（答案要包含正確的數字）、④目前有哪些"
            "分類/標籤（把便利貼清單裡出現過的標籤都列出來）。",
            "",
            "重點：要理解使用者的『意圖與需求』，不是只做字面關鍵字比對。"
            "使用者常會用間接、口語、情境式的說法——例如「我口渴了」其實是想"
            "找『喝的東西』（對應到飲料類的便利貼）、「想放鬆一下」是想找休閒"
            "或娛樂類的項目、「晚餐吃什麼」是想找食物/餐廳類的項目。請推斷"
            "背後真正想找的主題，把『語意上相關』的便利貼都算進去，即使那則"
            "便利貼的文字裡完全沒有出現問題中的任何字詞。標籤/分類名稱是很強"
            "的線索：如果問題明顯指向某個主題，該主題分類底下的便利貼通常都算"
            "相關。",
            "",
            "但也要精準：ids 只放『真的和使用者意圖相關』的便利貼，並依相關"
            "程度由高到低排序（最相關的編號放最前面）。不要為了湊數把只沾到"
            "一點邊、其實不符合需求的也放進去；沒有任何相關的就給空陣列。",
            "",
            ...
            '（*、-、#）>", "ids": [<跟這個問題意圖相關的便利貼編號，依相關程度'
            "由高到低排序；問題如果跟特定便利貼無關（例如純粹問分類清單），"
            "列出全部便利貼的編號；真的沒有相關的便利貼就給空陣列 []>], "
            '"list_tags": <如果問題是④「目前有哪些分類/標籤」這種要求列出全部'
            "標籤清單的問題就給 true，其餘情況一律給 false——是 true 的話"
            "answer 只要簡短帶一句話就好（例如「目前的標籤如下：」），不用"
            "自己在 answer 裡把標籤一個一個條列出來，程式會自動接上正確、"
            "有編號的完整清單>}",
            "",
            f"使用者的問題：{query}",
            "",
    ...
    def parse_ai_search_response(self, response: str, notes: list) -> tuple:
        ...
        if data is not None and isinstance(data, dict):
            ...
            if not isinstance(raw_ids, list):
                raw_ids = []
            # 保留模型回的順序（prompt 要求依相關程度由高到低排）——去重、
            # 濾掉超出範圍的編號，用 dict.fromkeys 保序去重。
            by_index = {i: note for i, note in enumerate(notes, start=1)}
            ordered = list(dict.fromkeys(
                int(i) for i in raw_ids
                if (isinstance(i, int) and not isinstance(i, bool))
                or (isinstance(i, str) and i.strip().isdigit())
            ))
            matched = [by_index[i] for i in ordered if i in by_index]
